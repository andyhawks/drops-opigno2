    ...
    if (!empty($gid)) {
      ...
      if ($current_step !== NULL) {
        ...
        if (!$is_evaluated) {
          ...
          $message = $this->t('Module "@module" needs manual evaluating', ['@module' => $current_step['name']]);
          $url = '/module/' . $module_id . '/results/' . $status_id;
          // Get user managers.
        ...
        if ($score >= $current_step['required score']) {
          $message = $this->t('Successfully completed module "@module" in "@name"', ['@module' => $current_step['name'], '@name' => $group->label()]);
          $url = '/group/' . $gid;
          opigno_set_message($uid, $message, $url);

          if ($is_last) {
            $message = $this->t('Congratulations! You successfully finished the training "@name"', ['@name' => $group->label()]);
            $url = '/group/' . $gid;
            opigno_set_message($uid, $message, $url);
          }
        }
        ...
        if ($current_step['typology'] == 'Module' && $opigno_module->badge_active->value) {
          ...
          if ($badge_notification) {
            $message = $this->t('You earned a badge "@badge" in "@name"', ['@badge' => $badge_notification, '@name' => $group->label()]);
            $url = '/group/' . $gid;
            opigno_set_message($uid, $message, $url);
          }
        }
